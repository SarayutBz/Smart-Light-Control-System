...
relay_pins = {
    "room8": 12,  # ห้อง อ.สนิท   
    "room9": 16,  # ห้อง อ.สมนึก
    "room10": 21,  # ห้อง อ.ก่องกาญ
    "room11": 20,  # ห้อง ห้องว่าง
    "room12": 23,  # ห้อง อ.ภานุ
    "room13": 24,  # ห้อง อ.โจ
}

# ✅ กำหนดขา GPIO สำหรับ PIR Sensor (เซ็นเซอร์จับความเคลื่อนไหว)
pir_to_relay = {
    22: "room8",   # อ.สนิท -> Relay GPIO 12
    27: "room9",   # อ.สมนึก -> Relay GPIO 16
    13: "room10",  # อ.ก่องกาญ -> Relay GPIO 18
    10: "room12",  # อ.ภานุ -> Relay GPIO 23
    19: "room13"   # อ.โจ -> Relay GPIO 24
}
...
for pir in pir_to_relay.keys():
    if pir == 27:  # ✅ ตรวจสอบว่าขา PIR เป็น 21 หรือไม่
...
def publish_status(room):
    ...
    client.publish(f"raspi/2/{room}/status", json.dumps(status))
...
for room in relay_pins:
    client.subscribe(f"raspi/2/{room}/control")
    client.subscribe(f"raspi/2/{room}/mode")
    client.subscribe(f"raspi/2/{room}/status/request")
